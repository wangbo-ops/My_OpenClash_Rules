...
- PayPal 规则放在两条 private 规则之后，优先于后续通用直连和代理规则。
- 新增 `💳 PayPal` 手动组。候选顺序按原需求保留：全球直连第一，支持所有地区组、家宽组及全部单个节点。
- 新增 `🏠 家宽节点` 手动组：匹配家宽、家庭宽带、住宅及不区分大小写的 Residential。
- 香港、美国、日本、新加坡、台湾、韩国组改为 `select`，保留最新上游匹配正则，移除自动测速尾部参数。
- `♻️ 自动选择` 保持原样。地区组仍包含该地区的家宽节点；家宽组可包含不同地区，依据名称分类。
...
- `paypal.choices`：PayPal 候选组及顺序。第一项为没有历史选择时的初始选择。
